...
#SBATCH --job-name=evaluate_type_paragraph
#SBATCH --output=%x.out
#SBATCH --error=%x.err
...
RAW_VCF="$HOME/gt/paragraph/exp2/nodup_01_02_d${d}/result/genotypes.vcf.gz"
REF_FA="$HOME/data/reference/DM8.1_genome.fasta.gz" # 参考序列
OUT_DIR="$HOME/gt/paragraph/exp2/nodup_01_02_d${d}/evaluate_type"
REF_PREFIX="DM8"
OUT_PREFIX="nodup_01_02_sv"
...
fix_paragraph_vcf "$RAW_VCF" "$OUT_DIR/${OUT_PREFIX}_final.vcf.gz" "$REF_PREFIX"
...
for sample in nodup_01 nodup_02; do
    ...
    bcftools view -s "$sample" -c 1 "${OUT_PREFIX}_50bp.vcf.gz" -Oz -o "$SAMPLE_QUERY_ALL"
