...
#SBATCH --job-name=evaluate_type_evg
#SBATCH --output=%x.out
#SBATCH --error=%x.err
...
QUERY_VCF_DIR="$HOME/gt/evg/exp2/nodup_01_02_evg_d${d}/merge"
OUT_DIR="$HOME/gt/evg/exp2/nodup_01_02_evg_d${d}/evaluate"
...
for sample in nodup_01 nodup_02; do
    ...
    SAMPLE_FIXED_QUERY="$QUERY_VCF_DIR/${sample}.vcf.gz"
    ...
    for type in TOTAL INS DEL INV; do
        ...
        if [ "$type" == "TOTAL" ]; then
            bcftools view "$SAMPLE_FIXED_TRUTH" -Oz -o "$sub_truth"
            bcftools view "$SAMPLE_FIXED_QUERY" -Oz -o "$sub_query"
        else
            # 按类型过滤
