...
#SBATCH --job-name=evaluate_type_swave
#SBATCH --output=%x.out
#SBATCH --error=%x.err
...
RAW_QUERY_VCF="$HOME/graph/swave/exp2/nodup_01_02_swave/fixed_sv/nodup_01_02_50bp_std.converted.vcf"
OUT_DIR="$HOME/graph/swave/exp2/nodup_01_02_swave/evaluate_type_sv"
...
for sample in nodup_01 nodup_02; do
    ...
    for type in TOTAL INS DEL INV; do
        ...
        truvari bench -b "$sub_truth" -c "$sub_query" -f "$REF_FA" -o "$tru_out_dir" \
            --sizemin 50 --sizemax 1000000 --pctseq 0.0 --pctsize 0.5 --refdist 1000 --no-ref a --typeignore
