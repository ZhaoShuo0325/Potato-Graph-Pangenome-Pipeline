...
TRUTH_VCF_DIR="$HOME/merge_test/00_data/01_realSV"
QUERY_VCF_DIR="$HOME/merge_test/02_graph/01_merge/genotyping/results"
OUT_DIR="$HOME/merge_test/02_graph/01_merge/genotyping/results/evaluate"
REF_FA="/public/home/zhaoshuo/work1/data/reference/DM8.1_genome.ori.chr.fa"
...
for sample in $(cat $LIST); do
    fix_truth_vcf "$TRUTH_VCF_DIR/${sample}_fixed.vcf.gz" "$OUT_DIR/${sample}_truth_all.vcf.gz"
done
...
for sample in $(cat $LIST); do
    ...
    SAMPLE_FIXED_QUERY="$QUERY_VCF_DIR/${sample}.combine.vcf.gz"
