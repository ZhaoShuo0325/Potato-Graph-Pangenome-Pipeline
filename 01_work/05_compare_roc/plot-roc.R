...
dat.plot <- ggplot(dat.roc, aes(x = FPR, y = TPR, color = aligner, label = mq)) +
    ...
    geom_point(aes(size = Positive + Negative), alpha = 0.5) +
    geom_text_repel(data = subset(dat.roc, mq %% 20 == 0),
                    size = 4, # 标签字体调大
    ...
    scale_size_continuous("Read Count", guide = guide_legend(ncol = 3)) +
    scale_x_log10(limits = c(1e-8, 1e-1), breaks = 10^seq(-8, -1), oob = squish) +
    ...
    theme(
        ...
        # === 图例设置：放在右侧内部，且去除了所有边框和底色 ===
        legend.position = c(0.95, 0.3),            # 放置在右下方
        legend.justification = c("right", "bottom"), 
        legend.background = element_blank(),      # 清除图例背景底色
        legend.box.background = element_blank(),  # 清除图例盒子边框
        legend.key = element_blank(),             # 清除图例图标背景
        legend.text = element_text(size = 16),
        legend.title = element_text(size = 18, face = "bold"),
        legend.margin = margin(5, 5, 5, 5)        # 内边距
    ) +
    labs(title = title, x = "FPR (False Positive Rate)", y = "TPR (True Positive Rate)")

filename <- commandArgs(TRUE)[2]
ggsave(filename, height = 6, width = 8)
